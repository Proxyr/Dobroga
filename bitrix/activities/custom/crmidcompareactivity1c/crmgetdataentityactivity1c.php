...
class CBPcrmidcompareactivity1c extends CBPActivity
{
	...
	protected function getEntityData()
	{
		...
		//если гуид не заполнен - проверяем id
		if (!($this->EntityGUID)){
			...
			case 'CONTACT':
				{
					if (isset($guid)){
						$objectResult = CCrmContact::getListEx(
							['ID' => 'DESC'],
							['=UF_ID1C' => $guid, 'CHECK_PERMISSIONS' => 'N']
						);
					}
					else
					{
		...
		$workflowVariables, $currentValues = null, $formName = '')
	{
		...
		if (!is_array($currentValues))
		{
			...
			if (is_array($currentActivity['Properties']))
			{
				...
				$currentValues['EntityGUID'] = $currentActivity['Properties']['EntityGUID'];
		...
		&$workflowVariables, $currentValues, &$errors)
	{
		...
		if (empty($currentValues['EntityGUID'])){
		...
		$properties['EntityGUID'] = $currentValues['EntityGUID'];
	...
	protected static function getEntityFields($entityType)
	{
		...
		{
			if (!$field['Editable']&!$field['Name']=='ID')
				continue;
